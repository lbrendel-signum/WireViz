...
def parse(
    ...
    for connection_set_index, connection_set in enumerate(connection_sets):
        try:
            ...
            # Add context about which connection set failed
            raise _add_context_to_error(e, f"Error in connections[{connection_set_index}]")
...
def _get_yaml_data_and_path(inp: Union[str, Path, dict]) -> tuple[dict, Path]:
    ...
    if not isinstance(inp, Dict):  # received a str or a Path
        ...
        try:
            ...
            if yaml_path:
                raise type(e)(f"YAML parsing error in file '{yaml_path}': {e}")
            else:
                raise type(e)(f"YAML parsing error: {e}")
    else:
